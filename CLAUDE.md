...
Tavern is a self-hosted, web-first, Discord-shaped community app for small private communities, tabletop RPG groups, and board gamers. Self-hosted with opt-in federation (Phases 1-3: peering + remote-user identity + federated channel messages/edits/deletes/reactions via `FEDERATION_ENABLED`, off by default — DMs, presence, invites, and voice still local). No public discovery, no monetization, no paid AI dependencies. A local Trust & Safety Core handles moderation.
